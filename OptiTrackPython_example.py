...
import time
from OptiTrackPython import OptiTrackPythonClient


class TestOptiTrackPythonClient(object):
    def __init__(self,
                 client_ip='192.168.2.101',
                 server_ip="192.168.2.100",
                 multicast_address="239.255.42.99",
                 rigidbody_names2track=["RigidBody 1"]):
        ...
        self.streamingClient = OptiTrackPythonClient(client_ip,
                                                     server_ip,
                                                     multicast_address)
    ...
    def receiveRigidBodyFrame(self, id, position, rotation, rigidBodyDescriptor):
                    ...
                    if id == rigidBodyDescriptor[rbname][0]:
                        ...
                        if self.lock_opti.acquire(False):
                            try:
                                ...
                                self.optitrack_reading[rbname] = [position,
                                                                  rotation]
                            finally:
